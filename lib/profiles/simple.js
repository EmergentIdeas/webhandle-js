...
const initializeNotFoundResponse = require('../initializers/not-found-response')
...
function simpleInit() {
	...
	// We put this at the start of static routers so that we've let all the user auth code run first
	initializeRequestLogging(this.routers.staticServers)
	...
	// what happens if we end in an error (500) or not found (404)
	initializeNotFoundResponse(this)
